...
def main_honeypot_endpoint(req: HackathonRequest):
    """
    Main endpoint for hackathon evaluation.
    Accepts the exact schema specified in requirements and returns minimal response.
    """
    ...
    all_text = " ".join([m["content"] for m in memory + [{"content": latest_text}]])
    intel = extract_intelligence(all_text)
    intel_payload = _build_intel_payload(intel)
    ...
    # Send callback if scam detected and sufficient engagement
    _maybe_send_callback(
        session_id=session_id,
        scam_detected=result["is_scam"],
        total_messages=total_messages,
    ...
    # Return minimal required response format
    return HackathonResponse(status="success", reply=reply_result["reply"])

# --------------------------------------------------
# Hackathon schema endpoint (alternative path)
# --------------------------------------------------
